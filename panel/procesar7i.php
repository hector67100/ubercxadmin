...
$query = "SELECT image FROM provincia WHERE id = '$idCategoria'";
...
if (isset($_FILES['foto']) && $_FILES['foto']['error'] == 0) {
    ...
    $query = "UPDATE provincia SET name = '$nombres', status = '$visible' WHERE id = '$idCategoria'";
    mysqli_query($link, $query) or die(mysqli_error($link));
}

header("Location: provincia.php"); // Redirigir a la página de categorías
mysqli_close($link);
?>