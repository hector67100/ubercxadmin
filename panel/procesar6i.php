...
$query = "SELECT image FROM tags WHERE id = '$idCategoria'";
...
if (isset($_FILES['foto']) && $_FILES['foto']['error'] == 0) {
    ...
    if (in_array($fotoType, $allowedMimeTypes)) {
        ...
        if (move_uploaded_file($fotoTmpPath, $uploadFilePath)) {
            ...
            $query = "UPDATE tags SET name = '$nombres', image = '$fileName' WHERE id = '$idCategoria'";
    ...
    $query = "UPDATE tags SET name = '$nombres' WHERE id = '$idCategoria'";
    mysqli_query($link, $query) or die(mysqli_error($link));
}

header("Location: tags.php"); // Redirigir a la página de categorías
mysqli_close($link);
?>